...
def _normalize_extrato_df(df_raw: pd.DataFrame) -> pd.DataFrame:
    """
    Padroniza colunas do extrato Sicoob para: DATA | DOCUMENTO | HISTÓRICO | VALOR.
    Remove linhas de 'saldo', cria _hist_norm, _data e _valor_raw.
    """
    df = df_raw.copy()
    # Garante o cabeçalho exatamente neste formato
    df.columns = ["DATA", "DOCUMENTO", "HISTÓRICO", "VALOR"]
