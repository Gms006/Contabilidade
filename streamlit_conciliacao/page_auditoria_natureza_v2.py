...
def gerar_observacao(tipo_conta, natureza_encontrada, eh_redutora):
    ...
    if natureza_encontrada == 'D':
        return f'{redutora_txt}{tipo_conta} com saldo devedor (esperado: credor)'
    else:
        return f'{redutora_txt}{tipo_conta} com saldo credor (esperado: devedor)'
...
def analisar_balancete_arquivo(arquivo_bytes):
    ...
    if faltando:
        raise ValueError(f'Colunas nao encontradas: {faltando}')
...
def gerar_relatorio_bytes(contas):
    ...
    ws.freeze_panes = 'A2'
    ws.auto_filter.ref = f'A1:I{len(contas) + 1}'
    ...
    if arquivo_upload is not None:
        try:
            ...
            if contas_incorretas:
                ...
                st.write(f'**Exibindo {len(contas_filtradas)} de {len(contas_incorretas)} contas**')

                cores_tipo = {'Ativo': 'blue', 'Passivo': 'violet', 'Despesa': 'red', 'Receita': 'green'}

                for conta in contas_filtradas:
                    codigo_fmt = str(conta.codigo).replace('.', '').replace(',', '').strip()
                    cor = cores_tipo.get(conta.tipo_conta, 'gray')

                    with st.container(border=True):
                        col_desc, col_tipo = st.columns([5, 1])
                        with col_desc:
                            titulo = conta.descricao
                            if conta.eh_redutora:
                                titulo += ' [REDUTORA]'
                            st.markdown(f'**{titulo}**')
                        with col_tipo:
                            st.markdown(f':{cor}[**{conta.tipo_conta}**]')
                        ...
                        with col2:
                            ...
                            st.code(conta.classificacao)
                        with col3:
                            st.caption('SALDO ATUAL')
                            if conta.saldo_atual < 0:
                                st.markdown(f':red[**{fmt_valor(conta.saldo_atual)}**]')
                            else:
                                st.markdown(f':green[**{fmt_valor(conta.saldo_atual)}**]')
                        with col4:
                            st.caption('NATUREZA')
                            st.markdown(f':red[**{conta.natureza_encontrada}**] para :green[**{conta.natureza_correta}**]')

                        st.error(conta.observacao, icon=None)
