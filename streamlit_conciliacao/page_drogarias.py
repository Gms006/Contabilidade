...
def mostrar_pagina_drogarias():
    ...
    # Botao de download CSV no topo - DESTAQUE
    ...
    with tabs[0]:
        ...
        with col2:
            ...
            # Checkbox para padronizacao automatica
            if STANDARDIZE_AVAILABLE:
                auto_padronizar = st.checkbox(
                    "Padronizar extrato automaticamente",
                    value=True,
                    key="drog_auto_pad",
                    help="Se o extrato estiver no formato bruto do banco, ele sera padronizado automaticamente"
                )
            else:
                auto_padronizar = False

            up_ext = st.file_uploader(
                "Extrato Sicoob.xlsx",
                type=["xlsx", "xlsm"],
                key="drog_ext",
                help="Extrato bancario do Sicoob (pode ser bruto ou ja padronizado)"
            )
            ...
            if up_ext:
                ...
                if auto_padronizar and STANDARDIZE_AVAILABLE:
                    try:
    ...
    if up_pag and up_ext and up_contas:
        try:
            # Usa extrato padronizado se disponivel
            if 'drog_extrato_padronizado' in st.session_state:
                up_ext_to_use = st.session_state['drog_extrato_padronizado']
            else:
                up_ext_to_use = up_ext

            df_pag, cols_pag = load_payments(up_pag)
            df_ext_saidas, df_ext_entradas, cols_ext = load_bank_split(up_ext_to_use)
            ...
            if btn:
                if validation_result and validation_result.get("tem_bloqueadores"):
                    st.error("**Nao e possivel conciliar!** Corrija os problemas na aba Avancado.")
                    st.stop()
    ...
    with tabs[1]:
        if 'drog_matches' in st.session_state:
            ...
            with col4:
                ...
                st.metric("Taxa Conciliacao", f"{pct:.1f}%")
            ...
            with col2:
                ...
                if not pend_data["unmatched_extrato"].empty:
                    ...
                    st.success("Todas as saidas foram identificadas!")
            ...
            if pct_conciliacao >= 95:
                st.success(f"**Excelente**: Taxa de conciliacao de {pct_conciliacao:.1f}%")
                st.caption("A maioria dos lancamentos foram conciliados com sucesso.")
            elif pct_conciliacao >= 85:
                st.warning(f"**Bom**: Taxa de conciliacao de {pct_conciliacao:.1f}%")
                st.caption("Ha algumas pendencias que merecem atencao.")
            else:
                st.error(f"**Critico**: Taxa de apenas {pct_conciliacao:.1f}%")
                st.caption("Muitos lancamentos nao foram conciliados. Revise os dados.")
    ...
    with tabs[2]:
        if 'drog_df_pag' in st.session_state:
            ...
            if 'drog_df_ext_saidas' in st.session_state:
                with st.expander("Extrato - Saidas (completo)", expanded=False):
